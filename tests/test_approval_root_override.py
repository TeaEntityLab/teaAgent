...
def test_explicit_root_overrides_saved_state() -> None:
    ...
    assert str(tui.root) == str(Path(explicit_root).resolve())


def test_saved_root_cannot_override_explicit_root() -> None:
    ...
    assert str(tui_default.root) == str(Path(saved_root).resolve())
    ...
    assert str(tui_explicit.root) == str(Path(explicit_root).resolve())


def test_tui_root_command_sets_explicit_flag() -> None:
    tui = TeaAgentTUITestHelper(root='/tmp/original-root')

    from teaagent.tui._commands import _cmd_root

    _cmd_root(tui, ['/tmp/new-root'])
    assert tui._root_explicit
    assert str(tui.root) == str(Path('/tmp/new-root').resolve())


def test_explicit_root_persists_across_state_load() -> None:
    ...
    assert str(tui.root) == str(Path('/tmp/explicit-root-3').resolve())
