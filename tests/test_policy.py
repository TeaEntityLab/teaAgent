...
class ParsePermissionModeTests(unittest.TestCase):
    def test_parses_valid_modes(self) -> None:
        ...
        self.assertIs(parse_permission_mode("workspace-write"), PermissionMode.WORKSPACE_WRITE)
        ...
        self.assertIs(parse_permission_mode("danger-full-access"), PermissionMode.DANGER_FULL_ACCESS)
...
class ApprovalPolicyTests(unittest.TestCase):
    def test_non_destructive_tool_always_allowed(self) -> None:
        for mode in PermissionMode:
            ...
            policy.assert_allowed(tool_name="workspace_read_file", call_id="c1", destructive=False)
    ...
    def test_read_only_blocks_destructive(self) -> None:
        ...
        with self.assertRaises(ToolPermissionError) as ctx:
            policy.assert_allowed(tool_name="workspace_write_file", call_id="c1", destructive=True)
    ...
    def test_workspace_write_allows_file_write_tools(self) -> None:
        ...
        for tool_name in {"workspace_write_file", "workspace_apply_patch", "workspace_edit_at_hash"}:
    ...
    def test_workspace_write_blocks_shell_destructive(self) -> None:
        policy = ApprovalPolicy(permission_mode=PermissionMode.WORKSPACE_WRITE)
        with self.assertRaises(ToolPermissionError) as ctx:
            policy.assert_allowed(tool_name="workspace_run_shell_mutate", call_id="c1", destructive=True)
    ...
    def test_prompt_mode_with_allow_all_destructive_passes(self) -> None:
        ...
        policy.assert_allowed(tool_name="workspace_write_file", call_id="any", destructive=True)

    def test_prompt_mode_with_approved_call_id_passes(self) -> None:
        policy = ApprovalPolicy(approved_call_ids=frozenset({"call-42"}))
        policy.assert_allowed(tool_name="workspace_write_file", call_id="call-42", destructive=True)

    def test_prompt_mode_without_approval_blocks(self) -> None:
        policy = ApprovalPolicy()
        with self.assertRaises(ToolPermissionError) as ctx:
            policy.assert_allowed(tool_name="workspace_write_file", call_id="unapproved", destructive=True)
