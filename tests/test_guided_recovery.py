...
def test_format_advice_json(formatter):
    ...
    assert json_output["recommended_strategy"]["command"] == "teaagent audit view --run test-run-456"
...
def test_format_command_substitution(formatter):
    ...
    assert json_output["recommended_strategy"]["command"] == "teaagent run --resume my-run-id --timeout-extended"
