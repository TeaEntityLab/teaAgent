...
def _install_candidate_skill(base: Path, rel_dir: str, name: str, body: str) -> Path:
    ...
    digest = canonical_content_digest(
        substrate=PersistenceSubstrate.SKILL_CANDIDATE, payload=payload
    )
...
def test_governance_candidate_installed(tmp_path: Path) -> None:
    ...
    root.mkdir()
    skill_dir = _install_candidate_skill(
        root, '.config/agent/skills', 'my-skill', 'Body ' * 20
    )
    source_dir = skill_dir.parent

    status = classify_governance_status(
        skill_dir=skill_dir, source_dir=source_dir, root=root
    )
...
def test_explain_with_lifecycle_tracker_emits_transitions(tmp_path: Path) -> None:
    ...
    transition_calls = [kw for kw in call_kwargs if kw.get('to_state') == 'activated']
    assert len(transition_calls) == 2  # one per loaded skill


def test_explain_lifecycle_tracker_sets_discovered_and_activated(
    tmp_path: Path,
) -> None:
    _install_skill(tmp_path, '.config/agent/skills', 'alpha', 'Alpha ' * 50)
...
def test_explain_governance_candidate_installed_in_output(tmp_path: Path) -> None:
    _install_candidate_skill(
        tmp_path, '.config/agent/skills', 'managed', 'Managed ' * 20
    )
