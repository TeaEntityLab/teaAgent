...
def test_pending_approval_can_resume_with_recorded_observation() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        adapter = FakeAdapter(
            [
...
def test_mcp_http_initialize_and_tool_call() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        (root / 'hello.txt').write_text('hello', encoding='utf-8')
        ...
        try:
            session_id = _initialize(host, port)
            body = json.dumps(
                {
                    'jsonrpc': '2.0',
                    'id': 2,
...
def _initialize(host: str, port: int) -> str:
...
def _request(
    host: str, port: int, body: bytes, *, headers: dict[str, str]
) -> tuple[int, bytes]:
    conn = http.client.HTTPConnection(host, port, timeout=5)
    try:
        all_headers = {
