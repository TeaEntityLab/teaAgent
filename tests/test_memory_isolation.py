...
class MemoryIsolationTests(unittest.TestCase):
    def test_memory_entry_with_branch_and_run_id(self) -> None:
        ...
        self.assertEqual(entry.branch_name, 'teaagent-sandbox-123-optA')
        self.assertEqual(entry.run_id, 'run-123')

        entry_dict = entry.to_dict()
        self.assertEqual(entry_dict['branch_name'], 'teaagent-sandbox-123-optA')
        self.assertEqual(entry_dict['run_id'], 'run-123')

    def test_memory_catalog_add_with_branch(self) -> None:
    ...
    def test_memory_catalog_delete_by_branch(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(entries[0].branch_name, 'branch-b')

    def test_memory_catalog_delete_by_run_id(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            entries = catalog.list()
            self.assertEqual(len(entries), 1)
            self.assertEqual(entries[0].run_id, 'run-2')

    def test_memory_catalog_quarantine_by_branch(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(len(quarantine_entries), 2)
            for entry in quarantine_entries:
                self.assertEqual(entry['branch_name'], 'branch-a')
                self.assertEqual(entry['provenance']['reason'], 'Test quarantine')

    def test_memory_entry_from_payload_with_branch(self) -> None:
    ...
    def test_memory_entry_from_payload_without_branch(self) -> None:
        ...
        entry = memory_entry_from_payload(payload)
        self.assertIsNotNone(entry)
        self.assertIsNone(entry.branch_name)
        self.assertIsNone(entry.run_id)

    def test_memory_entry_from_payload_invalid_branch(self) -> None:
    ...
    def test_memory_catalog_readonly_prevents_delete(self) -> None:
    ...
    def test_memory_catalog_readonly_prevents_quarantine(self) -> None:
    ...
    def test_memory_catalog_delete_nonexistent_branch(self) -> None:
    ...
    def test_memory_catalog_quarantine_preserves_provenance(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(quarantine_data['content'], 'Important content')
            self.assertEqual(quarantine_data['tags'], ['important'])
            self.assertEqual(quarantine_data['branch_name'], 'branch-a')
            self.assertEqual(quarantine_data['run_id'], 'run-123')
            self.assertTrue(quarantine_data['quarantine'])
            self.assertEqual(quarantine_data['provenance']['reason'], 'Test quarantine')
            self.assertIn('quarantined_at', quarantine_data['provenance'])
