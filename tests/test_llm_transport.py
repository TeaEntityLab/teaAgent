...
def test_build_ssl_context_returns_none_without_tls_env() -> None:
    with patch.dict(os.environ, {}, clear=True):
        assert build_ssl_context_from_env() is None


def test_build_ssl_context_loads_ca_bundle_from_requests_env() -> None:
    context = MagicMock()
    with (
...
def test_build_ssl_context_loads_client_cert_and_key() -> None:
    context = MagicMock()
    with (
        patch.dict(
            os.environ,
...
def test_transport_post_json_passes_ssl_context_when_tls_env_set() -> None:
    context = MagicMock()
