...
def test_plan_generator_steps(plan_generator):
    ...
    assert any("implement" in desc.lower() or "fix" in desc.lower() for desc in step_descriptions)
    assert any("test" in desc.lower() or "verify" in desc.lower() for desc in step_descriptions)
...
def test_plan_step_permission_modes(plan_generator):
    ...
    read_only_steps = [step for step in plan.steps if step.permission_mode == "read_only"]
