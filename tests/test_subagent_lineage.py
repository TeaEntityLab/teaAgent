...
def test_run_subagent_records_lineage_and_audit() -> None:
    with TemporaryDirectory() as td:
        ...
        assert child_config.max_iterations == 2, (
            'child budget should inherit per-call overrides'
        )
        assert child_config.max_tool_calls == 1
        # initial_context_extra is passed as keyword arg
        call_kwargs = run_mock.call_args.kwargs
        assert (
            call_kwargs['initial_context_extra']['subagent_lineage']['parent_run_id']
            == 'parent-xyz'
        )

        assert payload['run_id'] == 'child-abc'
        lineage = payload['lineage']
        assert lineage['parent_run_id'] == 'parent-xyz'
        assert lineage['def_name'] == 'generic'
        assert lineage['depth'] == 1
        assert lineage.get('batch_index') == 1
        assert lineage['isolation'] == 'shared'

        session = manager._sessions['child-abc']
        assert session.parent_run_id == 'parent-xyz'
        assert session.depth == 1


def test_subagent_tool_uses_parent_run_context() -> None:
    with TemporaryDirectory() as td:
        root = Path(td)
        (root / '.teaagent').mkdir()
        ...
        assert captured['parent_run_id'] == 'parent-from-runner'


def test_subagent_batch_returns_ordered_lineage() -> None:
    with TemporaryDirectory() as td:
        root = Path(td)
        (root / '.teaagent').mkdir()
        registry = ToolRegistry()
        ...
        assert result['lineage'][0]['batch_index'] == 0
        assert result['lineage'][1]['batch_index'] == 1
        assert result['lineage'][0]['parent_run_id'] == 'parent-batch'
        assert result['results'][0]['run_id'] == 'run-0'
        assert result['results'][1]['run_id'] == 'run-1'


def test_parent_run_context_default_empty() -> None:
