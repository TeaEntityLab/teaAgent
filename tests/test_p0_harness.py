...
class P0HarnessTests(unittest.TestCase):
    ...
    def test_destructive_tool_requires_exact_call_approval(self) -> None:
        ...
        pending = [event for event in audit.events if event.event_type == "tool_call_pending_approval"]
    ...
    def test_destructive_tool_can_be_approved_by_hitl_handler(self) -> None:
        ...
        def decide(context):
            if not context["observations"]:
                return ToolRequest(tool_name="pilot_echo", arguments={"value": "approved"}, call_id="call-hitl")
        ...
        self.assertIn("tool_call_approved", [event.event_type for event in audit.events])
    ...
    def test_denied_hitl_approval_fails_without_tool_execution(self) -> None:
        ...
        def decide(_context):
            return ToolRequest(tool_name="pilot_echo", arguments={"value": "denied"}, call_id="call-deny")
