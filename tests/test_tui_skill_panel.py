...
def test_cmd_skills_returns_valid_json() -> None:
...
def test_cmd_skills_includes_loaded_count() -> None:
...
def test_cmd_skills_includes_governance_status() -> None:
    ...
    loaded = parsed.get('loaded', [])
    assert len(loaded) >= 1
    assert 'governance_status' in loaded[0]
    assert loaded[0]['governance_status'] == 'candidate_installed'


def test_print_state_panel_skills_section() -> None:
