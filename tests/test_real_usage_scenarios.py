...
def _env_for_opencodezen() -> dict[str, str]:
    return {'OPENCODEZEN_API_KEY': _opencodezen_api_key() or ''}


def test_subagent_permission_capping_safety() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch('teaagent.chat_agent.run_chat_agent', fake_run_chat_agent):
            ...
            assert captured_sub_config is not None
            # Verify permission mode of the subagent is capped at workspace-write
            assert captured_sub_config.permission_mode == 'workspace-write'


def test_subagent_batch_concurrency_and_cancellation() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch.object(manager, 'run_subagent', fake_run_subagent):
            ...
            assert 'Simulated task error' in res['results'][1]['message']


def test_tui_interactive_approval_denial_flow() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert tui.handle_command('ask write to file.txt')
...
@pytest.mark.skip(reason='Requires live model and API key; skip for offline runs')
def test_live_model_smoke_opencodezen_go() -> None:
    ...
    with patch.dict(os.environ, _env_for_opencodezen()):
        ...
        assert res['provider'] == 'opencodezen-go'
        assert res['model'] == 'deepseek-v4-flash'
        assert 'ok' in res['content'].lower()
