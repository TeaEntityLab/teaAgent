...
def test_controller_displays_error_on_failure(tmp_path: Path) -> None:
    ...
    output = output_buffer.getvalue()
    assert 'pop from empty list' in output or '[failed' in output, (
...
def test_controller_no_output_on_empty_final_answer(tmp_path: Path) -> None:
    """Test that an empty final answer completes without printing RunResult repr (CG-01)."""
    output_buffer = io.StringIO()
    ...
    result = controller.execute_task(
        task='empty task',
        config=config,
        adapter=FakeAdapter(['{"type":"final","content":""}']),
    )
