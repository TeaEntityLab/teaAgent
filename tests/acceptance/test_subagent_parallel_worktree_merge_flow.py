...
def test_parallel_worktree_children_expose_lineage_for_parent_review() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for result, expected_wt in zip((first, second), worktrees, strict=True):
            ...
            assert result['lineage']['worktree_path'] == expected_wt
            assert result['lineage']['parent_run_id'] == 'parent-merge'
            assert 'parent review' in result['message']


def test_worktree_child_review_patch_can_be_checked_and_applied() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        _init_repo(root)
        config = ChatAgentConfig(root=root, enable_subagent=True)
        ...
        with patch('teaagent.chat_agent.run_chat_agent', side_effect=fake_child_run):
        ...
        assert result['review']['review_id'] == 'child-review'
        assert 'feature.txt' in result['review']['changed_files']
        assert (root / result['review']['patch_path']).is_file()
        assert not (root / result['lineage']['worktree_path']).exists()
        ...
        assert apply_code == 0
        assert (root / 'feature.txt').read_text(
            encoding='utf-8'
        ) == 'child worktree change\n'
