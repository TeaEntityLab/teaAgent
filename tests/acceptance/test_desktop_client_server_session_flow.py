...
class DesktopClientServerSessionFlowTests(unittest.TestCase):
    def test_mcp_http_client_server_and_cli_session_list(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('workspace_read_file', tools)
            self.assertFalse(result['isError'])
            self.assertIn('client-server', result['content'][0]['text'])
