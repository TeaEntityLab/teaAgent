...
def test_provider_registry_matches_docs_and_cli_output() -> None:
    ...
    assert provider_count_match, "README must include provider count in architecture diagram"
    ...
    doctor_providers = _extract_doc_providers(r"teaagent doctor model ([a-z0-9-]+)", cli_doc)
    assert doctor_providers == set(providers)

    smoke_providers = _extract_doc_providers(r"teaagent model smoke ([a-z0-9-]+)", cli_doc)
