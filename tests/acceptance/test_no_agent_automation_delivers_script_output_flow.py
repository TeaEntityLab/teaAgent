...
def test_collector_failure_does_not_wake_agent(tmp_path: Path) -> None:
    collector = tmp_path / 'collector.py'
    collector.write_text(
        'import json, sys\n'
        'print(json.dumps({"wake_agent": True, "summary": "should not wake"}))\n'
        'sys.exit(2)\n',
        encoding='utf-8',
    )
    add_out = io.StringIO()
    with redirect_stdout(add_out):
        add_code = main(
            [
                ...
                'add',
                'bad-collector',
                'Run collector and wake the agent only when collector succeeds.',
                '--schedule',
                'every 30m',
                '--collector-command',
                f'{sys.executable} {collector}',
                '--acceptance-criteria',
                'Collector exits 0 before any agent wake.',
                '--root',
                str(tmp_path),
            ]
        )
    ...
    assert payload['collector']['exit_code'] == 2
    bg_dir = tmp_path / '.teaagent' / 'background'
    assert not list(bg_dir.glob('*.json')) if bg_dir.is_dir() else True
