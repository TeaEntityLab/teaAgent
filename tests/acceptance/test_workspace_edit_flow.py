...
def test_hash_read_edit_git_test_and_diff_summary() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert exit_code == 0
        assert target.read_text(encoding='utf-8') == 'Hello from TeaAgent\n'
        assert payload['status'] == 'completed'
        assert 'Diff summary:' in payload['final_answer']
        assert payload['audit_summary']['destructive_tool_calls'] == 2
        assert payload['audit_summary']['tool_names'] == [
