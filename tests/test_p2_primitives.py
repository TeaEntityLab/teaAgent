...
def build_registry(*, destructive: bool = False) -> ToolRegistry:
    ...
    registry.register(
        ...
        annotations=ToolAnnotations(read_only=not destructive, destructive=destructive, idempotent=True),
...
class P2PrimitiveTests(unittest.TestCase):
    def test_graph_rag_traverses_relations_to_documents(self) -> None:
        ...
        graph.add_document(Document(doc_id="doc-1", text="Alice owns Acme in logistics", source="graph"))
        graph.add_edge(GraphEdge(source="alice", relation="owns", target="acme", document_ids=("doc-1",)))
        graph.add_edge(GraphEdge(source="acme", relation="operates", target="logistics", document_ids=("doc-1",)))
    ...
    def test_managed_agent_readiness_flags_missing_hitl_for_destructive_tool(self) -> None:
        ...
        self.assertTrue(any("needs HITL" in finding.message for finding in report.findings))
        ...
        results = assess_provider_portability(
            [
                ProviderProfile(
                    name="portable",
                    model="model-a",
                    capabilities=frozenset({"tool_calling", "structured_output", "system_prompt", "prompt_caching"}),
