...
class ParallelExperimentsIntegrationTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.stack = ParallelExperimentStack(root=self.temp_dir, run_id=self.run_id, options=self.options)
    ...
    def test_experiment_isolation(self) -> None:
        ...
        subprocess.run(["git", "checkout", optA_branch], cwd=self.temp_dir, check=True, capture_output=True)
        (self.temp_dir / "test.txt").write_text("optA content", encoding="utf-8")
        subprocess.run(["git", "add", "."], cwd=self.temp_dir, check=True, capture_output=True)
        subprocess.run(["git", "commit", "-m", "optA change"], cwd=self.temp_dir, check=True, capture_output=True)
        
        # Modify optB
        optB_branch = results["optB"].branch_name
        subprocess.run(["git", "checkout", optB_branch], cwd=self.temp_dir, check=True, capture_output=True)
        (self.temp_dir / "test.txt").write_text("optB content", encoding="utf-8")
        subprocess.run(["git", "add", "."], cwd=self.temp_dir, check=True, capture_output=True)
        subprocess.run(["git", "commit", "-m", "optB change"], cwd=self.temp_dir, check=True, capture_output=True)
