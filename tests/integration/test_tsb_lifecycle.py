...
from teaagent.tsb_format import TSBBuilder, TSBManifest, TSBMetadata, TSBVerifier
...
def skill_path(temp_dir):
    """Create a sample skill directory with test files."""
    ...
    (skill / 'utils' / 'helper.py').parent.mkdir(parents=True, exist_ok=True)
    (skill / 'utils' / 'helper.py').write_text(
        'def helper():\n    pass\n', encoding='utf-8'
    )
    return skill
...
def audit_log_path(temp_dir):
    ...
    audit_events = [
        ...
        },
        {
            ...
            'payload': {'tool': 'write_file', 'path': 'test.py'},
            'prev_hash': 'hash1',
        },
    ]
    audit_log_content = '\n'.join(json.dumps(event) for event in audit_events)
    audit_log.write_text(audit_log_content, encoding='utf-8')
    return audit_log
...
def key_path(temp_dir):
    ...
    key = temp_dir / 'test_key'
    key.write_text('mock_key_content', encoding='utf-8')
    return key


def test_full_tsb_lifecycle_without_signature(
    temp_dir, skill_path, audit_log_path
) -> None:
    ...
    output_path = temp_dir / 'test.tsb'
    manifest = builder.build_tsb(output_path, metadata, skip_audit_verification=True)

    # Verify manifest structure
    assert isinstance(manifest, TSBManifest)
    assert manifest.metadata.skill_name == 'test-skill'
    assert manifest.metadata.tsb_version == '1.1'
    assert len(manifest.files) == 3  # SKILL.md, main.py, utils/helper.py
    ...
    assert 'TSB verification successful' in message
    ...
    assert (extract_path / 'SKILL.md').exists()
    assert (extract_path / 'main.py').exists()
    assert (extract_path / 'utils' / 'helper.py').exists()

    # Verify file contents
    assert (extract_path / 'SKILL.md').read_text(
        encoding='utf-8'
    ) == '# Test Skill\n\nA test skill for TSB integration.'
    assert (extract_path / 'main.py').read_text(
        encoding='utf-8'
    ) == "def main():\n    print('Hello')\n"


def test_tsb_with_path_aware_hashing(temp_dir, skill_path, audit_log_path) -> None:
    ...
    (skill_path / 'main.py').rename(skill_path / 'renamed.py')
    output_path2 = temp_dir / 'test2.tsb'
    ...
    assert manifest.attestation.bundle_hash != manifest2.attestation.bundle_hash, (
        'Bundle hashes should differ when file paths change'
    )


def test_tsb_verification_with_tampered_manifest(
    temp_dir, skill_path, audit_log_path
) -> None:
    ...
    assert not is_valid, 'Verification should fail with modified file content'
    assert 'Bundle hash mismatch' in message


def test_tsb_path_traversal_protection_in_extract(
    temp_dir, skill_path, audit_log_path
) -> None:
...
def test_tsb_deterministic_hash_across_multiple_builds(
    temp_dir, skill_path, audit_log_path
) -> None:
    ...
    # Build twice
    output_path1 = temp_dir / 'test1.tsb'
    output_path2 = temp_dir / 'test2.tsb'
    ...
    assert manifest1.attestation.bundle_hash == manifest2.attestation.bundle_hash, (
