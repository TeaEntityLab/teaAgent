...
class TSBLifecycleIntegrationTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        (self.skill_path / 'main.py').write_text(
            "def main():\n    print('Hello')\n", encoding='utf-8'
        )
        (self.skill_path / 'utils' / 'helper.py').parent.mkdir(
            parents=True, exist_ok=True
        )
        (self.skill_path / 'utils' / 'helper.py').write_text(
            'def helper():\n    pass\n', encoding='utf-8'
        )
    ...
    def test_tsb_with_path_aware_hashing(self) -> None:
        ...
        manifest2 = builder.build_tsb(
            output_path2, metadata, skip_audit_verification=True
        )
    ...
    def test_tsb_verification_with_tampered_manifest(self) -> None:
        ...
        self.assertFalse(
            is_valid, 'Verification should fail with modified file content'
        )
    ...
    def test_tsb_deterministic_hash_across_multiple_builds(self) -> None:
        ...
        manifest1 = builder.build_tsb(
            output_path1, metadata, skip_audit_verification=True
        )
        manifest2 = builder.build_tsb(
            output_path2, metadata, skip_audit_verification=True
        )
