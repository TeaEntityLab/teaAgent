...
def test_execute_task_with_fake_adapter() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        adapter = FakeAdapter(['{"type":"final","content":"hello from controller"}'])
        ...
        if result.run_result.final_answer:
            assert 'hello' in result.run_result.final_answer.content
        assert result.cost_cents >= 0


def test_execute_task_accepts_config_overrides() -> None:
...
def test_execute_task_emits_answer_when_requested() -> None:
...
def test_execute_task_handles_failed_run() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert result.run_result.status.startswith('failed'), (
            f'Expected failed status, got: {result.run_result.status}'
        )


def test_undo_last_run_without_journal_returns_false() -> None:
...
def test_get_session_cost_tracks_across_tasks() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert len(controller.session_state.observations) == 2


def test_get_session_cost_display_format() -> None:
...
def test_execute_task_handles_run_store_factory() -> None:
...
def test_execute_task_records_audit_events() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert len(run_ended) > 0


def test_session_cost_accumulates_via_sessions_state() -> None:
    """SessionState starts at zero and accumulates across multiple rounds."""
    state = SessionState()
    assert state.session_cost_cents == 0.0

    # Simulate two tasks completing with known cost
    state.session_cost_cents += 50.0
    state.session_cost_cents += 75.0
    assert state.session_cost_cents == 125.0


# ---------------------------------------------------------------------------
# TuiFullPipelineTests
# ---------------------------------------------------------------------------


def test_run_agent_task_with_fake_adapter() -> None:
...
def test_run_agent_task_with_final_answer_json_output() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert payload['final_answer'] == 'json-result'


def test_run_agent_task_tracks_cost() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch.object(
            ...
            assert tui2._session_cost_cents >= cost1


def test_run_agent_task_handles_run_failed() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        joined = '\n'.join(output)
        assert 'failed' in joined.lower() or 'error' in joined.lower(), (
            f'Expected failure indicator in output: {joined}'
        )


def test_run_agent_task_with_clarify() -> None:
    ...
    assert 'needs_clarification' in '\n'.join(output)


def test_run_agent_task_with_clarify_concrete_proceeds() -> None:
...
def test_approval_handler_approves() -> None:
...
def test_approval_handler_denies() -> None:
...
def test_progress_sink_records_events() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        joined = '\n'.join(output)
        assert 'iter 1' in joined
        assert 'tool: workspace_read_file' in joined
        assert 'tool ok: workspace_read_file' in joined


def test_budget_prompt_handler_accepts() -> None:
...
def test_budget_prompt_handler_rejects() -> None:
...
def test_run_agent_task_sets_last_run_id() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert tui.last_run_id is not None
        assert isinstance(tui.last_run_id, str)
        assert len(tui.last_run_id) > 0


# ---------------------------------------------------------------------------
# ModelDecisionEngineEdgeTests
# ---------------------------------------------------------------------------


def test_plain_text_answer_fallback_returns_final_answer() -> None:
...
def test_plain_text_answer_fallback_returns_none_with_observations() -> None:
...
def test_plain_text_answer_fallback_returns_none_for_workspace_task() -> None:
...
def test_looks_like_simple_answer_task_true() -> None:
    """_looks_like_simple_answer_task returns True for Q&A-style questions."""
    assert _looks_like_simple_answer_task('What is Python?')
    assert _looks_like_simple_answer_task('How do I create a loop?')
    assert _looks_like_simple_answer_task('Explain async/await')
    assert _looks_like_simple_answer_task('Tell me about machine learning')
    assert _looks_like_simple_answer_task('Can you tell me about AI?')


def test_looks_like_simple_answer_task_false() -> None:
    """_looks_like_simple_answer_task returns False for workspace file tasks."""
    assert not _looks_like_simple_answer_task('Read the file src/main.py')
    assert not _looks_like_simple_answer_task('Fix the bug in tests/')
    assert not _looks_like_simple_answer_task('Edit README.md')
    assert not _looks_like_simple_answer_task('Run the test suite')
    assert not _looks_like_simple_answer_task('Commit and push changes')
    assert not _looks_like_simple_answer_task('')


def test_looks_like_plain_text_answer_true() -> None:
    """_looks_like_plain_text_answer returns True for natural language answers."""
    assert _looks_like_plain_text_answer(
        'Python is a high-level programming language that is widely used.'
    )


def test_looks_like_plain_text_answer_false() -> None:
    """_looks_like_plain_text_answer returns False for JSON-like or short answers."""
    assert not _looks_like_plain_text_answer('short')
    assert not _looks_like_plain_text_answer('{"key": "value"}')
    assert not _looks_like_plain_text_answer('[1, 2, 3]')
    assert not _looks_like_plain_text_answer('```json\n{}\n```')


def test_parse_model_decision_returns_tool_request() -> None:
    ...
    assert result.tool_name == 'workspace_read_file'
    assert result.call_id == 'c1'


def test_parse_model_decision_returns_final_answer() -> None:
...
def test_decide_with_fake_llm_adapter() -> None:
    ...
    assert decision.content == 'decision-made'


def test_decide_with_tool_request_from_fake_llm() -> None:
    ...
    assert decision.tool_name == 'workspace_read_file'


def test_decide_falls_back_to_plain_text_after_retries() -> None:
...
def test_decide_raises_for_workspace_task_after_max_retries() -> None:
    ...
    assert 'parsing failed' in str(ctx.value)


def test_decide_with_stream_on_chunk_called() -> None:
...
def test_decide_accumulates_cost_in_context() -> None:
    ...
    engine.decide(context)
    assert '_cost_cents' in context
    assert '_input_tokens' in context
    assert '_output_tokens' in context


# ---------------------------------------------------------------------------
# PermissionModeTransitionTests
# ---------------------------------------------------------------------------


def test_read_only_blocks_destructive_tools() -> None:
...
def test_read_only_allows_non_destructive_tools() -> None:
...
def test_workspace_write_allows_file_write_tools() -> None:
...
def test_workspace_write_blocks_shell_mutate() -> None:
...
def test_prompt_mode_requires_handler_for_destructive() -> None:
...
def test_prompt_mode_allows_non_destructive() -> None:
...
def test_allow_mode_allows_all_destructive() -> None:
...
def test_danger_full_access_allows_all() -> None:
...
def test_all_permission_modes_are_distinct() -> None:
    ...
    assert len(modes) == 5
    assert len({m.value for m in modes}) == 5
