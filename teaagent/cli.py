...
def build_parser() -> argparse.ArgumentParser:
    ...
    memory_list = memory_subparsers.add_parser(
        'list', help='List recent memory entries.'
    )
    memory_list.add_argument(
        '--root', default='.', help='Workspace root. Defaults to current directory.'
    )
    memory_list.add_argument(
        '--limit', type=int, default=20, help='Maximum memories to list.'
    )
    memory_list.set_defaults(func=memory_list_command)

    memory_search = memory_subparsers.add_parser(
        'search', help='Search memory entries.'
    )
    ...
    agent_run.add_argument(
        '--max-tool-calls', type=int, default=10, help='Maximum tool calls.'
    )
    agent_run.add_argument(
        '--clarify',
        action='store_true',
    ...
    agent_status = agent_subparsers.add_parser(
        'status', help='Show liveness status of a persisted run.'
    )
    ...
    agent_show = agent_subparsers.add_parser(
        'show', help='Show one persisted run JSONL record.'
    )
    ...
    graphqlite_doctor = doctor_subparsers.add_parser(
        'graphqlite', help='Check GraphQLite runtime availability.'
    )
    graphqlite_doctor.add_argument(
        '--database',
        default=':memory:',
        help='SQLite database path. Defaults to :memory:.',
    )
    ...
    model_doctor = doctor_subparsers.add_parser(
        'model', help='Check model provider configuration.'
    )
    ...
    providers = model_subparsers.add_parser(
        'providers', help='List configured provider names.'
    )
    providers.set_defaults(func=model_providers)

    smoke_model = model_subparsers.add_parser(
        'smoke', help='Run a minimal prompt against a provider.'
    )
    ...
    graphqlite_subparsers = graphqlite.add_subparsers(
        dest='graphqlite_command', required=True
    )

    query = graphqlite_subparsers.add_parser(
        'query', help='Execute a Cypher query against GraphQLite.'
    )
    ...
    smoke = graphqlite_subparsers.add_parser(
    ...
    ultrawork_subparsers = ultrawork.add_subparsers(
        dest='ultrawork_command', required=True
    )
    ...
    ultrawork_list = ultrawork_subparsers.add_parser(
        'list', help='List background workers.'
    )
    ultrawork_list.add_argument('--root', default='.', help='Workspace root.')
    ultrawork_list.set_defaults(func=ultrawork_list_command)

    ultrawork_show = ultrawork_subparsers.add_parser(
        'show', help='Show one worker record.'
    )
    ...
    ultrawork_stop = ultrawork_subparsers.add_parser(
        'stop', help='Stop a running worker.'
    )
    ...
    workspace_tools = workspace_subparsers.add_parser(
        'tools', help='List workspace tool metadata.'
    )
    workspace_tools.add_argument(
        '--root', default='.', help='Workspace root. Defaults to current directory.'
    )
