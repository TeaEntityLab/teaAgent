...
class EnvironmentManager:
    ...
    def provision_uv(self, spec: EnvironmentSpec) -> Lockfile:
        ...
        Returns:
            Generated lockfile.
        """
        print(f"[Loading...] Reading environment spec from teaagent.toml...")
        print(f"[Resolving...] UV virtual environment initialization...")
        ...
        if spec.packages:
            ...
            subprocess.run(
                [str(pip_path), "install"] + [f"{pkg.name}=={pkg.version}" if pkg.version else pkg.name for pkg in spec.packages],
                check=True,
            )
        
        # Generate lockfile
        print(f"[Locking...] Locking dependency hashes...")
        lockfile = generate_lockfile(spec, f"{sys.version_info.major}.{sys.version_info.minor}")
        write_lockfile(lockfile, self._lockfile_path)
        
        print(f"[✓] Hermetic environment locked & verified: teaagent.lock ({len(lockfile.entries)} packages).")
        return lockfile
    ...
    def provision_nix(self, spec: EnvironmentSpec) -> Lockfile:
        ...
        """
        print(f"[Loading...] Reading environment spec from teaagent.toml...")
        print(f"[Resolving...] Nix flake environment initialization...")
        ...
        try:
            ...
            print("[Error] Nix not found. Please install Nix: https://nixos.org/download.html")
            raise
        ...
        print(f"[Creating...] Nix flake environment...")
        subprocess.run(
            ["nix", "flake", "update"],
            check=True,
            cwd=self._root,
    ...
    def provision_docker(self, spec: EnvironmentSpec) -> Lockfile:
        ...
        """
        print(f"[Loading...] Reading environment spec from teaagent.toml...")
        print(f"[Resolving...] Docker environment initialization...")
    ...
    def verify(self) -> bool:
        ...
        print(f"[✓] Environment compliance: VALID")
        return True
