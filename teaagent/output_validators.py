...
class OutputValidator(ABC):
    ...
    @abstractmethod
    def validate(
        self, artifact_path: Path, source_metadata: dict
    ) -> ValidationResult:
...
class PromptInjectionValidator(OutputValidator):
    ...
    def validate(
        ...
        injection_patterns: list[str] = source_metadata.get(
            "injection_patterns", []
        )
        ...
        for pattern in injection_patterns:
            if pattern in content:
                ...
                if not quoted:
                    issues.append(
                        f"Injection pattern present but not quoted: "
                        f"'{pattern}'"
                    )
            else:
                issues.append(
                    f"Injection pattern not found in output: '{pattern}'"
                )
        ...
        return ValidationResult(
            ...
            passed=True,
            evidence=(
                f"All {len(injection_patterns)} injection pattern(s) "
                "safely quoted"
            ),
            severity="warning",
        )
