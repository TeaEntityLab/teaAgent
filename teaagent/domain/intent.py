"""Intent clarification layer for TeaAgent.

Domain reasoning lives here (A-P1-1); the root ``teaagent.intent`` module is
now a compat shim that re-exports this module's public API.

This module preserves the interactive clarification behavior added in U-P1-1
(the interactive prompting itself lives in the CLI handlers, which consume
``clarify_task`` / ``ClarificationResult`` from here).
"""
