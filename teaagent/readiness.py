...
def assess_managed_agent_readiness(
    *,
    ...
    if not tool_metadata:
        findings.append(
            ReadinessFinding('error', 'At least one registered tool is required.')
        )
    for tool in tool_metadata:
        if not tool.get('description'):
            findings.append(
                ReadinessFinding(
                    'error', f'Tool {tool.get("name")} is missing a description.'
                )
            )
        ...
        if annotations.get('destructiveHint') and not has_human_approval:
            findings.append(
                ReadinessFinding(
                    'error', f'Destructive tool {tool.get("name")} needs HITL.'
                )
            )
    if not has_external_state:
        findings.append(
            ReadinessFinding(
                'warning',
                'Long-lived runs need external state before managed runtime migration.',
            )
        )
