...
def audit_verify_command(args: argparse.Namespace) -> int:
    ...
    print(f'[Verifying...] Scanning audit events against genesis block...')
    ...
    print(f'[✓] Cryptographic Hash Chain: VALID (zero gaps, zero modifications, zero insertions).')
    ...
    if args.signature:
        ...
        if not key_path.exists():
            print_json({
                'status': 'error',
                'message': f'Signature key not found at {key_path}'
            })
            return 1
        ...
        try:
            ...
            if key_path.suffix in ['.pub', '']:
                ...
                signature_lines = [line for line in signature_output.split('\n') if line.startswith('-----BEGIN')]
                if signature_lines:
            ...
            print_json({
                'status': 'error',
                'message': f'Signature generation failed: {e}'
            })
            return 1
    ...
    print_json({
        'status': 'valid',
        'event_count': result.event_count,
        'signature_file': str(audit_log_path.with_suffix('.jsonl.sig')) if args.signature else None,
    })
    return 0
