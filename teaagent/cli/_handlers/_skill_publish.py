...
def skill_publish_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        output_path = (
            Path(args.output)
            if args.output
            else skill_path.parent / f'{skill_path.name}.tsb'
        )
        ...
        print('[Redacting...] Applying privacy filters to audit log...')

        use_sigstore = getattr(args, 'sigstore', False)
        identity_token = getattr(args, 'identity_token', None)

        if use_sigstore:
            print('[Signing...] Using Sigstore keyless signing...')
...
def skill_verify_tsb_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print('[Verifying...] Checking TSB integrity and attestation...')
        ...
        if offline:
            print('[Mode...] Offline verification (skipping Rekor/Fulcio)')
