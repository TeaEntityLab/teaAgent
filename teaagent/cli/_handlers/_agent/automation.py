...
from teaagent.cli._handlers._agent._automation_selectors import (
    _AmbiguousAutomationName,
    _resolve_automation_selector,
    _resolve_quarantined_selector,
)
...
def automation_add_command(args: argparse.Namespace) -> int:
    try:
        compute_next_run_at(args.schedule)
    except ValueError as exc:
        message = str(exc).replace(
            'unsupported schedule;',
            f'unsupported schedule {args.schedule!r};',
        )
