...
def benchmark_parallel_experiment_creation(count: int = 10) -> dict[str, float]:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        stack = ParallelExperimentStack(
            root=tmp_path, run_id='benchmark-exp', options=options
        )

        start = time.perf_counter()
        stack.start_all(auto_stash=False)
...
def benchmark_experiment_isolation(count: int = 5) -> dict[str, float]:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for opt in options:
            start = time.perf_counter()
            subprocess.run(
                ['git', 'checkout', results[opt].branch_name],
                cwd=tmp_path,
                check=True,
                capture_output=True,
            )
...
def print_benchmark_results() -> None:
    ...
    print(
        f'{"Count":<10} {"Creation (s)":<15} {"Cleanup (s)":<15} {"Avg Create (s)":<15} {"Avg Cleanup (s)":<15}'
    )
    print('-' * 80)
