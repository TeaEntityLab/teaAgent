...
class QualityMatrix:
                        ...
                        if 'passed' in line:
                            ...
                            for i, part in enumerate(parts):
                                if part.isdigit() and i > 0:
                                    passed = int(part)
                                    total = int(parts[i-2]) if i >= 2 and parts[i-2].isdigit() else passed
                                    results['test_pass_rate'] = (passed / total * 100) if total > 0 else 0.0
                                    break
            except (subprocess.TimeoutExpired, FileNotFoundError):
    ...
    def compare(self, results: List[Dict[str, Any]]) -> str:
        ...
        for result in results:
            ...
            if result['performance_ms'] > 0 and result['performance_ms'] != float('inf'):
...
def run_parallel_optimization(
    ...
    print(f"[Parallel Optimizer] Testing {len(strategies)} strategies: {', '.join(strategies)}")
    ...
    for branch_name in results.keys():
        print(f"[Parallel Optimizer] Evaluating {branch_name}...")
        # Checkout the branch
        subprocess.run(['git', 'checkout', branch_name], cwd=project_root, capture_output=True)
        
        # Evaluate
        result = quality.evaluate(branch_name)
        quality_results.append(result)
        
        print(f"[Parallel Optimizer] {branch_name}: "
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        print("Usage: python main.py <project_root> <strategy1,strategy2,...> <task_description>")
        sys.exit(1)
