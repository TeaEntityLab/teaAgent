...
def make_db(path: Path) -> None:
    ...
    con.execute("CREATE TABLE issues (id INTEGER PRIMARY KEY, title TEXT, open INTEGER)")
    con.executemany(
...
def demo() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        observations = [o for o in result.observations if o.get("tool_name") == "query_issues"]
        if observations:
