...
| U-P0-1 | UX | Replace the placeholder URL in the first-run welcome and standardize all four GitHub URLs to one canonical URL. | `teaagent/cli/_handlers/_misc.py:475`; `SUPPORT.md:18`; `teaagent/cli/__init__.py:359`; `docs/ops/deployment-guide.md:68` | The entire repository uses one GitHub URL; `rg "yourusername\|anomalyco"` has no matches in documentation or CLI code. |
...
| G-P1-3 | Governance | Emit a `multisig_fallback` audit event when multisig falls back because `agent_id` is missing. | `teaagent/approval_manager.py:449-454` | Fallback emits an audit event; a misconfigured session is detectable afterward. |
...
| U-P1-5 | UX | Remove `--no-tui` from all module documentation or implement it. | `docs/modules/cli/*`; `docs/modules/tui/inspection.md`; `docs/modules/chat_session_controller/api.md`; `docs/decisions/trade-offs.md:101` | Documentation matches code; either `rg --no-tui docs/` has no matches or the flag exists. |
...
| --- | --- | --- | --- |
| S-P2-1 | Security | When the audit-chain key is missing, emit a `missing_chain_key` warning failure instead of silently skipping HMAC. | `teaagent/audit_chain.py:439-457` |
| S-P2-2 | Security | Add an explicit `is_symlink()` check to `_assert_paths_in_workspace` (defense in depth). | `teaagent/approval_manager.py:1090-1141` |
| S-P2-3 | Security | Sanitize `run_id` to `[A-Za-z0-9._-]` for `GitBranchSandbox` branch names. | `teaagent/sandbox/_git_branch.py:119` |
| S-P2-4 | Security | Make the Bandit step in `ci.yml` blocking by removing the `\|\| echo` fallback. | `.github/workflows/ci.yml:201-203` |
| S-P2-5 | Security | Correct the `notify.py:42` docstring (`shell=True` -> `shell=False`); make `config_lint` warn about `allow_dev_signatures` outside development workspaces. | `teaagent/notify.py:42,138-141`; `teaagent/config_lint.py:102-108` |
| G-P2-1 | Governance | Gate use of `directory-snapshot` isolation for untrusted content behind an `--acknowledge-no-os-isolation` flag. | `teaagent/subagents/_isolation.py:287-298` |
| G-P2-2 | Governance | Remove `preapproved_call_ids` after the deprecation window and standardize on payload digest. | `teaagent/policy.py:115-121`; `teaagent/approval_manager.py:953-968` |
| G-P2-3 | Governance | Add iteration and tool-call warnings alongside `BudgetMonitor`'s existing cost monitoring. | `teaagent/budget_monitor.py:74-106` |
| G-P2-4 | Governance | Document the `scope` field taxonomy (`call_id`/`payload_digest`/`session`/`preset`). | `teaagent/runner/_core.py:730,742` |
| G-P2-5 | Governance | Make `max_skill_md_lines` an error for installed (candidate-provenance) skills; keep it a warning during development. | `teaagent/skill_review.py:187-193` |
| A-P2-1 | Architecture | Consolidate optional-extra aliases: fold `crypto`/`oauth`/`audit-encryption` into a single `crypto` extra. | `pyproject.toml:60-68` |
| A-P2-2 | Architecture | Split CLI handler god modules: `_ergonomics.py:1407`, `_doctor.py:1022`, `_agent/run.py:999`, `chat_repl.py:894`. | As listed |
| A-P2-3 | Architecture | Expand property-based testing with Hypothesis strategies for permission-mode transitions, budget enforcement, approval-hash exactness, and audit-chain invariants. | `tests/` |
| A-P2-4 | Architecture | Populate `tests/skills/` or remove the empty directory. | `tests/skills/` |
| A-P2-5 | Architecture | Reconcile `tests/e2e/` (one file) with 127 acceptance tests by moving it or defining an e2e contract. | `tests/e2e/` |
| U-P2-1 | UX | Retire or connect `chat_repl.py`; choose one to avoid divergence between tested and production behavior. | `teaagent/cli/_handlers/chat_repl.py:202-225` |
| U-P2-2 | UX | Align TUI parser flags with `run`/`chat` (add budget flags; default `--provider` from configuration). | `teaagent/cli/_misc_parsers/tui_parser.py:25` |
| U-P2-3 | UX | Fix undo-scope divergence and update the known-issues document. | `teaagent/tui/core.py:1063-1079` |
| U-P2-4 | UX | Improve `--task` ergonomics with an alias or CLI hint; prominently document the provider/task order flip. | `teaagent/cli/_agent_parsers.py:63-95` |
| U-P2-5 | UX | Minor: duplicate `undo` help line, `--chat-mode` docstring, `['*']` "Explicit current directory" comment. | `teaagent/tui/rendering.py:50,81`; `_chat.py:3,15`; `core.py:1329` |
...
1. **Sprint 1 (Trust repair)**: U-P0-1 (URL), U-P0-2 (TUI advertised semantics), G-P0-1 (schema) - achievable in 1-2 days and immediately improves trust.
2. **Sprint 2 (Security bypasses)**: S-P0-1 (AutoMode), S-P0-2 (HMAC), S-P0-3 (library audit) - requires design and tests, 3-5 days.
