...
| SEC-11 | Undo | `UndoJournal._PATH_WRITE_TOOLS` covers file tools only; `workspace_run_shell_mutate` not tracked — UI shows "undo available" but shell side-effects are unrecoverable | H | M | 6 | security | 2026-07-15 | **DOCUMENTED 2026-06-30 / disclosure expanded 2026-07-01 / boundary ratified by ADR-0042 2026-07-01** — reversibility is two-tier: when the git sandbox is active (`--git-sandbox` / consent auto-enable, `cli/_handlers/_agent/run.py:335-411`), `agent undo` prefers `GitBranchSandbox.rollback` (`preflight.py:90-114`), which `git reset --hard` + `git clean -fd` **does** reverse in-worktree shell mutations; the file-only `UndoJournal` fallback (non-git/declined) cannot, so it warns (`run_undo.py:73` `PARTIAL_UNDO_SHELL_WARNING`) and every reporting surface now discloses partiality (`RunEvidenceSummary.rollback_shell_partial` → receipt `available (partial — shell mutations not reversed)`; `RunStateSnapshot.undo_shell_partial`). Out-of-worktree/external effects (network, DB, `$HOME`/`/tmp`, deletions outside the tree) are out of scope for post-hoc reversal by design — routed to isolation (ADR-0020), never falsely claimed. Tests: `tests/integration/test_run_undo_shell_warning.py`, `tests/test_run_evidence_summary.py`, `tests/test_run_state_contract.py`, `tests/test_run_receipt.py` | P2 |
