...
| G6 | Tests prove behavior, not construction | Partial — typing complete, ratchet absolute, P0 flags cleared | 606 test files; all 458 backlog files classified 2026-09-13 (455 behavior / 82 contract / 39 adversarial / 31 lifecycle); `UNTYPED_BASELINE = 0` — `--fail-on untyped` in CI now fails on **any** new untyped file. All 5 P0-tier flagged files strengthened 2026-09-13 (construction-only asserts → behavior asserts; one permanently-skipped placeholder became a real RecursionError test). Residual: 66 construction-only / 0 high-risk files remain in lower tiers. Raw suite counts are not a quality claim per harness-first §4.2 |
