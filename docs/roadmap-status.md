...
| M5 | Ongoing | Prompt/runtime/model/provider gating, repo-map benchmarking, release evidence bundles | TBD | Partially fixed — repo-map corpus gated | Low | non-advisory release profile | Prompt/conversational regression suite and repo-map fixture corpus are in the release profile; remaining exit criteria are non-advisory model/provider regression evidence and release evidence bundle proof |
