...
| G6 | Tests prove behavior, not construction | Partial — typing complete, ratchet absolute | 606 test files; all 458 backlog files classified 2026-09-13 (455 behavior / 82 contract / 39 adversarial / 31 lifecycle); `UNTYPED_BASELINE = 0` — `--fail-on untyped` in CI now fails on **any** new untyped file. Residual: 71 construction-only / 5 high-risk files remain (assertion-strengthening is a separate lane). Raw suite counts are not a quality claim per harness-first §4.2 |
...
| ID | Work Item | Implementation Status | Scheduling | Confidence | Required Exit Evidence |
|----|-----------|-----------------------|------------|------------|------------------------|
| VND-001 | Live supervisor/monitor lane: trusted reviewer pass over the run trajectory (post-run first, in-run later) classifying events against a threat taxonomy; acceptance shaped on DeepMind's coverage/recall/time-to-response metrics | Absent — audit is post-hoc only; no trajectory reviewer exists | Proposed — owner approval + DR-006 gate classification (`governance-gap` arguable: monitoring is the missing layer all three vendors now ship) | Medium | Post-run reviewer pass over fixture audit log; taxonomy-classified findings; coverage metric reported |
| VND-002 | Insider-threat/misalignment threat class in `docs/threat-model.md` — agents as potential insider threats (DeepMind AI Control framing), not only external attackers | Absent — threat model covers external attackers | Proposed — docs-only, folds into next authorized docs slice | High | Threat-model section added; no behavior change |
