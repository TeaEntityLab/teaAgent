...
**Update 2026-09-13 (agentflow v8.2.0 survey delta + AGF-001 implementation):** re-surveyed `agfnow/agentflow` at `fcb6878` (v8.2.0) against the 2026-09-12 pin `b2935f5`; record `.teaagent/reviews/agentflow-structures-2026-09-12/delta-2026-09-13-v8.2.0.md` (gitignored). Prior consensus undisturbed — no agentflow structural machinery adopted. Intake section below: AGF-001 (cross-host skill conflict audit) **Complete** — `teaagent skill audit` landed under owner "implement governance gaps" direction; AGF-002 (guard incident citations) **Complete** — verified already present since `86fa25ea`; AGF-003 (run-store retention) and AGF-004 (per-skill version field) remain On Hold. **Next plan:** the only agent-actionable open items are AGF-003/004 (held pending organic traffic/consumer) and the DR-006 falsifier window closing 2026-09-22; everything else is owner-gated (ADR-0031 decision overdue since 2026-09-12, EFX live-provider proof, M4 dogfood booking, goal-text B-05/B-07/B-10, deletions B-06/B-09, B-11). No horizon, milestone, goal, or EFX status moved.
...
| G3 | One event spine | Partial — rescoped to observability | Only M0, M1-partial and M3 landed: `EventSpine` (`teaagent/runner/_events.py`, ADR-0032) has one interceptor (plan gate) and one consumer (`AuditLogger`), while `AgentRunner` still calls `audit.record()` directly and `HookRegistry`/`ContextBus` remain independent (`ContextBus` used in production by `teaagent/swarm.py`). M4 (approval/budget), M5 (hooks) and M6 (ContextBus/webhook consume spine) did not land per [M4](work-log/m4-budget-stays-inline-2026-06-13.md) and [M5](work-log/m5-hooks-observability-only-2026-06-13.md). The harness-first §2 failure mode "event spine becomes a fourth parallel system instead of replacing three" is ACCEPTED, not avoided; `scripts/validate_event_spine_wiring.py` as…
| G4 | Extensible by hooks, not forks | Partial — tool dispatch scope only | 8-event `HookRegistry` wired into tool dispatch (`teaagent/tools.py`); the 6 session-lifecycle events remain unwired in production. The ratified example "custom budget rule without touching `AgentRunner`" is NOT satisfied: budget enforcement is inline in `teaagent/runner/_core.py` |
| G5 | Docs corpus carries its weight | Partial — tier rule met, corpus grew | Constitution tier 8 ≤ 12 and aging dashboard green, but the corpus is **641** `docs/**/*.md` (inventory 640 rows: 8 constitution / 369 working / 263 archive) — growth from the 582-file baseline the harness-first doc called "costs more to keep truthful than it returns" (+10.1%). Prior claim of "~500-file corpus with 259 archive-tiered" was a misreport, corrected 2026-09-09; `tests/test_docs_consistency.py::test_roadmap_g5_corpus_counts_match_inventory` now guards these figures against drift; the aging dashboard now reports a Corpus Cost section (B-04, 2026-09-12): total vs 582 baseline, live-corpus count, and working-tier docs unreferenced by INDEX.md |
| G6 | Tests prove behavior, not construction | Partial — typing now honest | 606 test files; `classify_test_type` no longer defaults to `contract` — unmarked files classify `untyped` (B-03, 2026-09-12): 458 untyped / 132 behavior / 10 contract / 3 adversarial / 3 lifecycle at last audit. `--fail-on untyped` exists as the ratchet once the count reaches 0. Raw suite counts are not a quality claim per harness-first §4.2 |
