...
| --- | --- | --- | --- |
| G1 | Daily task without consulting docs | Unmeasured — no organic evidence | Friction log 5/5 owner entries closed, but all 12 entries are dated 2026-06-22 and there have been zero new entries since; no doc-lookup metric exists; `.teaagent/runs/runs-index.jsonl` records 415 runs over 2026-06-03→2026-08-31 comprising only 6 distinct synthetic benchmark/smoke prompts and zero organic owner tasks (2026-09-09 audit). Absence of friction is dormancy evidence, not ergonomics evidence |
...
| G6 | Tests prove behavior, not construction | Partial — typing is mostly fallback | 603 test files; only **17** carry an explicit type marker/comment and 136 are typed by path/filename heuristic, so **454 (75%) fall back to `scripts/audit_test_quality.py`'s default `contract`** classification. Raw suite counts are not a quality claim per harness-first §4.2 |
