...
| Approval UX parity | P1 | Same pending approval queue and selector semantics across CLI/TUI/IDE. | `test_approval_ux_parity_flow.py` | 🔄 In progress (shared queue + scoped grant + TUI `--resume`; dashboard parity pending) |
| Dashboard cockpit parity | P1 | Dashboard displays same blockers, costs, approvals, and recommendations. | `test_cockpit_snapshot_flow.py` | 🔄 In progress (`teaagent cockpit` JSON snapshot scaffold; no web UI yet) |
