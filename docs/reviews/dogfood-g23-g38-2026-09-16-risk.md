...
| G35 MCP unknown-tool → JSON-RPC `-32602`; any other per-request failure → `-32603`; session survives | A malformed client request or an internal failure no longer ends the session; a bug could be masked as a protocol error. | `-32602` is limited to the registry lookup; `-32603` carries the exception text in the frame and is logged via `logger.exception`, so evidence is preserved; tool execution errors keep the `isError` result contract. |
