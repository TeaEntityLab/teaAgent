...
### R1: Duplicate `handle_memory_failures`, `handle_pin`, etc. across `_chat.py` and `chat_commands.py`
...
### R2: `_chat.py::run_chat_repl` uses placeholder cost (`session_cost_cents += 10`)
...
### R3: `suspend_to_background` in `_chat.py` creates a git sandbox branch on dirty workspace
...
### R4: `ChatSessionController` silently swallows store persistence errors
...
### R5: `undo_last_run()` broad exception catch hides bugs
...
### R6: `complete_symbol` / `complete_file_path` in `_chat.py` are redundant with `chat_completion.py`
...
### R7: `chat_repl.py::run_chat_repl` cost variable desync
