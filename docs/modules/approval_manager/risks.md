...
## R1: `once`-scoped grants consumed before tool executes
...
## R2: `deny` decisions not persisted
...
## R3: `DANGER_FULL_ACCESS` bypasses all approval
...
## R5: File lock on `approvals.json` not held during TTY prompt
...
## R6: Approval UI shows diff in terminal — may expose secrets
**File**: `approval_ui.py`
...
## R7: TTL-expired grants silently removed on next access
