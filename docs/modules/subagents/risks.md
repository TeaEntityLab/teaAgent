...
### SUB-R-001: Path traversal in isolation session keys
...
### SUB-R-002: `directory-snapshot` workspace copy does not exclude `.env` / secrets
...
### SUB-R-003: `docker` isolation uses `ro` mount — subagent cannot write
...
### SUB-R-004: Sync approval polling re-reads disk every 0.25s
...
### SUB-R-005: HMAC secret in env var — cleartext
...
### SUB-R-006: `asyncio._lock` nested inside `threading._sync_lock` — deadlock risk
...
### SUB-R-007: `_build_registry_for` accesses private attribute `_parent_registry._tools`
...
### SUB-R-008: `TeamOrchestrator.run_team` is sequential, not truly concurrent
...
### SUB-R-009: `new_isolation_session_key` — `parent_run_id` truncated to 12 chars
...
### SUB-R-010: `capture_subagent_review` depends on `git add -N` — fails on non-git workspaces
**File**: `_review.py:57`
...
### SUB-R-011: `apply_patch` (review) path escape check
...
### SUB-R-012: `_load_simple_yaml` is a partial parser — can silently misparse valid YAML
