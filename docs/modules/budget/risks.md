...
### 1. Dead code in `BudgetMonitor.check()` — logic after early return
...
### 2. `BudgetMonitor` — `on_prompt` returning `False` does NOT halt execution
...
### 3. `RunBudget.validate()` is never called automatically
...
### 4. Pre-flight estimate is approximate
...
### 6. `CostTracker` silently drops malformed JSONL lines
...
### 7. `check_daily_cost_cap` uses `SystemExit`, not a catchable exception
...
### 8. `BudgetMonitor.from_env()` has no error handling for malformed env vars
