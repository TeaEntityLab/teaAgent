...
- Restricted Code Mode with AST allow-list validation and a replaceable backend interface. The default backend runs in a child-process sandbox with CPU-time, wall-clock timeout, and best-effort memory limits (RLIMIT_AS/RLIMIT_CPU via `resource`); `ContainerCodeModeBackend` can route execution through Docker/Podman-style runtimes with disabled networking, memory, CPU, and PID limits.
...
- Strong production sandboxing for Code Mode beyond the current container command boundary: seccomp profiles, V8 isolates, VM sandboxes, or a managed execution service.
