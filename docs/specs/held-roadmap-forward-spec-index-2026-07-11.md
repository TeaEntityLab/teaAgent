...
| --- | --- | --- | --- | --- |
| [H4 Policy/RBAC Promotion](rbac-shadow-to-enforce-promotion-spec-2026-07-11.md) | H4, ADR-0031 | `tests/test_h4_promotion_spec.py` — 7 | Promotion day replaces advisory-policy pin with enforce acceptance | RBAC enforces denials; approval policy always returns true even when its mode label says enforce (`h4_integration.py:72-120`). |
...
| [Consensus Disposition](consensus-validation-disposition-spec-2026-07-11.md) | ADR-0029 | `tests/test_consensus_disposition_spec.py` — 6 | Expiry chooses wire or delete | SUPERMAJORITY approves one YES when only one vote is cast; module has zero production importers. |
