...
| `teaagent/domain/workflow_engine.py` (multi-step, simulated execution) | 768 + 91 test | **No production caller**; `_execute_step` simulated | `legacy-competitive` | **Strongest deletion candidate** — importer sweep done 2026-09-13: zero production callers (only the root compat shim + tests reference it) |
