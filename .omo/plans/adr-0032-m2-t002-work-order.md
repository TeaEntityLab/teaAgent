...
> **Status**: Agent-safe implementation complete — 2026-07-22 | **Risk**: High | **Dependencies**: ADR32-M2-T001 (T001 already exists — `read_run_events_from_audit()` at `_events.py:217`) | **Human Review Required**: Yes (parity assertion)
...
**2026-07-22 execution note:** the safe local implementation landed in the
roadmap-job cutover: `EventDerivedEvidenceResult`,
`build_run_evidence_bundle_from_events()`, `build_run_evidence_bundle(...,
use_event_stream=True|False, raw_audit_events=...)`, `RUN_PAUSED` mapping, and
fold parity tests for success, failure, cancelled, pending approval, and
`run_paused`. The default production path was already typed-event based before
this note; this work adds the named compatibility API and explicit fallback flag.
